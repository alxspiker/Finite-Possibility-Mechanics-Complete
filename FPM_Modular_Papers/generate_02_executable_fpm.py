...
def manuscript(styles):
    ...
    s += [
        ...
        statement("Computational result", "The public runtime implements the finite periodic cubic geometry and local replenishment theorem directly. Floating-point residuals quantify numerical representation; they are not unrecorded energy channels.", styles),
    ]
    s += [
    ...
    s += [
        ...
        statement("Ledger reading", "The simulator does not conserve stored daemon energy at every tick. It conserves ordinary interior replenishment locally and the expanded signed ledger across all declared branches.", styles),
    ]
    s += [
        ...
        bullet("Lindblad correspondence: RMSE 1.79&times;10<sup>-102</sup> for the selected dephasing map.", styles),
        ...
        bullet("Point-Pair iteration: the fixed-point equation closes with zero residual in the evaluated binary64 arithmetic.", styles),
        ...
        bullet("SPARC gas-boundary bridge: archived benchmark metadata reports median RMSE 11.61 km/s; the live rerun is marked unavailable when the external tables are absent.", styles),
