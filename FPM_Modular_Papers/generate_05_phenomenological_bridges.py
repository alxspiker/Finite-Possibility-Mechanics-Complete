...
def manuscript(st):
    s = [
        ...
        body("This ceiling concerns the internal ratio L/L<sub>rest</sub>. A candidate lag extension introduces a progress accumulator z<sub>t+1</sub>=z<sub>t</sub>+L<sub>rest</sub>/L<sub>t</sub> and identifies a measured clock or decay process with integer crossings of z. A separate motion-to-ledger dictionary would then be required before a particle-lifetime or clock experiment can be registered.", st),
        statement("Status", "The numerical ceiling follows from the selected action bounds. A physical lag prediction requires a declared progress scheduler and an observable map; neither is supplied by the five foundational axioms alone.", st),
