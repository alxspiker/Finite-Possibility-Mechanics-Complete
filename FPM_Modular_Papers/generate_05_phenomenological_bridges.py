...
def manuscript(st):
    s = [
        ...
        h2("3.1 Candidate nine-channel equipartition state law", st),
        body("The axioms use normalized route cost and do not select a Kelvin-valued temperature. A candidate dimensional extension introduces an energy-per-route dictionary epsilon<sub>R</sub> and a thermodynamic state law. The directed 3 by 3 route ledger supplies n<sub>eff</sub>=9 native channels. The additional equipartition postulate assigns the bulk node capacity E<sub>max</sub> equally across those channels, so epsilon<sub>R</sub>=h/[n<sub>eff</sub> Delta t<sub>univ</sub>] and", st),
        equation("T<sub>erase</sub>=hE<sub>max</sub>/[n<sub>eff</sub>N k<sub>B</sub> ln(2) Delta t<sub>univ</sub>]=306.442 K.", st),
        body("A separate unweighted canonical-edge audit tests g<sub>ij</sub>=ln[P<sub>ij</sub>/P<sub>ji</sub>]=-&beta;<sub>L</sub>(L<sub>j</sub>-L<sub>i</sub>) on the first master-chain action state after the tick-zero truth-target update. Across 375 undirected edges it gives corr(Delta L,g)=0.00469040864, beta<sub>L</sub>=-0.0019023970513, and normalized residual ||g+beta<sub>L</sub> Delta L||<sub>2</sub>/||g||<sub>2</sub>=0.999993565405. The raw detailed-balance flux residual max|w<sub>i</sub>P<sub>ij</sub>-w<sub>j</sub>P<sub>ji</sub>| is 1.73x10<sup>-18</sup>, while the separate log-ratio activity-potential residual max|g-ln(w<sub>j</sub>/w<sub>i</sub>)| is 2.22x10<sup>-16</sup>. That activity potential is not collinear with route-cost differences: the declared state has no scalar canonical temperature conjugate to L. This does not select a replacement state law.", st),
        body("The equipartition rule is a proposed constitutive closure, not a consequence of Axioms A1-A5 alone. Substitution into the present dimensional gravitational formula gives G<sub>FPM</sub>=6.53990x10<super>-11</super> m<super>3</super> kg<super>-1</super> s<super>-2</super>, 2.0137 percent below the CODATA value. The residual is an unresolved discrepancy, not an inferred screening correction. Any adoption must state the route-energy dictionary, the thermodynamic state law, and the conditions under which no scalar temperature is defined.", st),
        ...
        body("The public reproducibility script downloads the SPARC master table and rotation-model archive from the cited Zenodo record [15] into a temporary directory, verifies their published MD5 checksums, and deletes them after the run. On its declared Q=1 sample of 99 galaxies with at least five valid rows, the gas-boundary FPM source functional gives median per-galaxy velocity RMSE 11.6065 km/s; the fixed comparison relation used by the script gives 11.7156 km/s. The comparison formula is defined by the archived implementation rather than by the 1983 modified-dynamics paper [7]. This is a reproducible retrospective scalar benchmark with no per-galaxy parameter fitting, not a tensor-morphology result, an independent confirmation, or a prospective prediction. The saved ledger records the source file checksums, selection rule, formulas, and metric.", st),
        ...
        body("Equivalently, substitution gives G<sub>FPM</sub>=(2/3)&zeta;<sup>2</sup>hc<sup>3</sup>/[m<sub>e</sub>&alpha;<sub>PP</sub>(&alpha;<sub>PP</sub>+9)N<sup>5</sup>k<sub>B</sub>T<sub>bath</sub>ln2]. Under the candidate nine-channel equipartition state law, T<sub>bath</sub>=306.442 K and the map returns", st),
        equation("G<sub>FPM</sub>=6.53990228&times;10<super>-11</super> m<sup>3</sup> kg<super>-1</super> s<super>-2</super>.", st),
        body("Compared with the CODATA value 6.67430(15)&times;10<sup>-11</sup> in the same units [13], the relative difference is 2.0137 percent. The nine-channel count is native to the carrier; equal thermal partition is the added physical postulate. No unregistered factor is introduced to eliminate the remaining difference.", st),

        h1("5. Internal lag and candidate muon-lag extension", st),
        body("Let L<sub>rest</sub> be the per-tick action required to maintain a reference process and L the current route cost. The internal lag and its inverse causal throughput are", st),
        equation("&gamma;<sub>FPM</sub>=L/L<sub>rest</sub>, &nbsp;&nbsp; &chi;<sub>throughput</sub>=L<sub>rest</sub>/L.", st),
        body("The quantity &chi;<sub>throughput</sub> decreases as route cost increases. It is an internal causal-throughput fraction, not a laboratory spatial velocity. The bounded action law gives", st),
        equation("1 &le; &gamma;<sub>FPM</sub> &le; &gamma;<sub>max</sub>=31.8738629.", st),
        h2("5.1 Candidate muon-lag extension", st),
        body("A candidate motion-to-ledger dictionary reserves v<sub>lab</sub> for an externally measured laboratory velocity and imports the ordinary low-load kinematic factor only as a dictionary:", st),
        equation("L(v<sub>lab</sub>)=min[L<sub>rest</sub>&gamma;<sub>kin</sub>(v<sub>lab</sub>),L<sub>max</sub>], &nbsp;&nbsp; &gamma;<sub>FPM</sub>(v<sub>lab</sub>)=min[&gamma;<sub>kin</sub>,&gamma;<sub>max</sub>].", st),
        body("The candidate scheduler is z<sub>t+1</sub>=z<sub>t</sub>+L<sub>rest</sub>/L<sub>t</sub>. If muon decay is postulated to have a constant hazard per unit internal progress and the measured rest lifetime tau<sub>mu</sub> is used only to set that hazard, the conditional survival law is", st),
        equation("P<sub>survive</sub>(t|v<sub>lab</sub>)=exp[-t/(tau<sub>mu</sub>&gamma;<sub>FPM</sub>(v<sub>lab</sub>))].", st),
        body("This gives a ceiling at v<sub>*</sub>=0.9995077253c, p<sub>*</sub>/(mc)=31.85817, and tau<sub>max</sub>=70.0263 microseconds for tau<sub>mu</sub>=2.1969811 microseconds [16]. The supplied historical storage-ring reference gamma=29.33 lies below the conditional ceiling, so it does not distinguish this extension from the imported low-load kinematic map. The public feasibility package records no machine-readable controlled lifetime series above the threshold; a documented 10-50 GeV/c muon test-beam capability is not itself a decay-lifetime dataset.", st),
        body("The carrier phase update exp(-i theta L) and the clock increment L<sub>rest</sub>/L have opposite L-dependence. The runtime verifies their coexistence with unitary phase evolution and unchanged Born probabilities for a uniform phase rotation; that is an implementation consistency check, not evidence that either update represents a muon observable.", st),
        statement("Status", "The finite internal-lag ratio follows from the selected action bounds. The motion dictionary, decay-hazard identification, and rest-lifetime calibration are candidate extension commitments. Their capped high-gamma curve is a conditional competing prediction, not a result of Axioms A1-A5 alone.", st),
        ...
        equation("q=1/{1+exp[10(max(E<sub>A</sub>,E<sub>B</sub>)/(0.20E<sub>max</sub>)-0.60)]}.", st),
        body("Its sharpness, midpoint, and scale are constitutive inputs, and it describes joint-resolution activation at low stored energy rather than exhaustion of a paid link. The code's geometric correlation is -cos(a-b); the paper's -cos[2(a-b)] form requires an explicit setting convention, for example theta<sub>code</sub>=2 theta<sub>lab</sub>.", st),
        h2("8.3 Candidate torsion-refresh postulate", st),
        body("A resource-exhaustion mechanism requires an active maintenance operation even when the stored torsion configuration is pure gauge. A candidate postulate is that an experimentally usable shared link executes one refresh transaction per active independent antisymmetric generator per tick. For n<sub>A</sub> generators its maintenance cost is C<sub>link</sub>=n<sub>A</sub>c<sub>0</sub>. With a one-generator link, c<sub>0</sub>=0.05, and a symmetric available action A<sup>(0)</sup>=0.5 subject to the declared depletion law, the conditional snap threshold is", st),
        equation("B<sub>*</sub>=[A<sup>(0)</sup>/(n<sub>A</sub>c<sub>0</sub>)]<sup>(4/3)</sup>-1=20.5443469.", st),
        body("This threshold follows after the refresh postulate and availability map are adopted; it is not a result of the existing zero-cost pure-gauge link. The corresponding Bell transition must specify the laboratory control variable, preparation distribution, and no-signalling constraints.", st),
        ...
        h1("10. Candidate extension programs and empirical risk", st),
        ...
        bullet("<b>Nine-channel equipartition extension:</b> an energy-to-route dictionary and equal thermal partition across the nine directed channels determine a Kelvin-valued temperature and propagate directly into G and mass-equivalent scales.", st),
        bullet("<b>Torsion extension:</b> a refresh postulate and a control-variable map determine whether a shared link has a resource-dependent survival threshold.", st),
        bullet("<b>Lag extension:</b> a progress scheduler and clock dictionary determine whether gamma_FPM maps to a measured rate or lifetime.", st),
        ...
        bullet("Construct a dynamical cosmological transfer model before treating acoustic structure as a physical prediction.", st),
        bullet("Adopt and test a progress scheduler plus a concrete clock or particle observable for gamma_FPM.", st),
        bullet("Adopt a torsion-refresh postulate, specify its laboratory resource variable, and test the predicted CHSH transition while preserving no-signalling.", st),
        ...
        h1("11. Conclusion", st),
        body("FPM's bridge programme is an auditable dependency stack. The foundational ledger supplies local transport, signed accounting, finite allocation, and fixed action bounds. Temperature, physical link maintenance, clock progress, and cosmological transfer dynamics are candidate extension programs with explicit new commitments. Calibrated and correspondence-level numerical comparisons remain distinct from physical predictions.", st),
        statement("Central conclusion", "A physical bridge is valid only at the level warranted by its declared dependencies. The extension programs make their additional assumptions, derived conditional consequences, and failure conditions explicit.", st),
        ...
        reference("[15] SPARC Galaxy Rotation Curve database, Zenodo record 16284118 (2025). <a href='https://doi.org/10.5281/zenodo.16284118'>https://doi.org/10.5281/zenodo.16284118</a>", st),
        reference("[16] Particle Data Group, <i>Muon listing</i>, 2025 update. <a href='https://pdg.lbl.gov/2025/listings/rpp2025-list-muon.pdf'>https://pdg.lbl.gov/2025/listings/rpp2025-list-muon.pdf</a>", st),
    ]
    return s
